...
main(int argc, char *argv[])
{
  if (argc != 7)
  ...
  else
    {
      ...
      model->baseNetwork->modules[model->baseNetwork->size - 1]->infoIof.format
          = BINARY;
      model->baseNetwork->modules[model->baseNetwork->size - 1]->infoIof.takeWriteFile(
          activeFileName);
      ...
      cout << "Finish after " << difftime(end, start) / 60 << " minutes"
          << endl;
      model->baseNetwork->modules[model->baseNetwork->size - 1]->infoIof.freeWriteFile();
      delete model;
    }
  return 0;
}
